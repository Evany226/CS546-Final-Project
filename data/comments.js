...
const getCommentById = async (commentId) => {
  ...
  const commentCollection = await comments();
  let commentById = await commentCollection.findOne({
...
const getCommentsByListing = async (listingId) => {
  ...
  if (!commentsByListing)
    throw new Error(
      `Could not get all comments from listing ${listingById.listingName}`
    );
...
const getCommentsByUser = async (userId) => {
  ...
  if (!commentsByUser)
    throw new Error(
      `Could not get all comments from user ${userById.username}`
    );
...
const updateComment = async (commentId, updateObject) => {
  ...
  if (updateObjectKeys.includes("review")) {
    ...
    if (updateObject.review < 1 || updateObject.review > 5)
      throw new Error("Review must be a number between 1 and 5");
